...
o1_df=pd.DataFrame(result, columns =['あなた','user1','user2','user3','user4','user5'])
...
# In[72]:
gya_df=gya_df.drop_duplicates(subset=['あなた','user1','user2','user3','user4','user5'])
...
for i in range(gya_df.shape[0]):
    x = gya_df.iloc[i,6:36].values
    # print(np.sum(x),np.var(x))
    gya_df.iloc[i,36] = float(np.sum(x))
...
for i in range(0,len(fin_df)):
    if((max(fin_df.iloc[i,6:11])>=24)and(max(fin_df.iloc[i,11:16])>=24)and(max(fin_df.iloc[i,16:21])>=24)and(max(fin_df.iloc[i,21:26])>=24)and(max(fin_df.iloc[i,26:31])>=24)and(max(fin_df.iloc[i,31:36])>=24)
    and(max(fin_df.iloc[i,11],fin_df.iloc[i,16],fin_df.iloc[i,21],fin_df.iloc[i,26],fin_df.iloc[i,31])>=24)and(max(fin_df.iloc[i,6],fin_df.iloc[i,17],fin_df.iloc[i,22],fin_df.iloc[i,27],fin_df.iloc[i,32])>=24)
    and(max(fin_df.iloc[i,7],fin_df.iloc[i,12],fin_df.iloc[i,23],fin_df.iloc[i,28],fin_df.iloc[i,33])>=24)and(max(fin_df.iloc[i,8],fin_df.iloc[i,13],fin_df.iloc[i,18],fin_df.iloc[i,29],fin_df.iloc[i,34])>=24)
    and(max(fin_df.iloc[i,9],fin_df.iloc[i,14],fin_df.iloc[i,19],fin_df.iloc[i,24],fin_df.iloc[i,35])>=24)and(max(fin_df.iloc[i,10],fin_df.iloc[i,15],fin_df.iloc[i,20],fin_df.iloc[i,25],fin_df.iloc[i,30])>=24)):
...
# 対象とする列のリスト
target_cols = ['user1', 'user2', 'user3', 'user4','user5']
...
e=unique_rows_df.iloc[0,5]
st.text(f"あなたは{a}さん、{b}さん、{c}さん、{d}さん、{e}さんとマッチしました")

st.text(f"{a}さんの詳細")
f=mbti_df.iloc[a-1,0:4]
st.text(f)
st.text("")
st.text(f"{b}さんの詳細")
g=mbti_df.iloc[b-1,0:4]
st.text(g)
st.text(f"{c}さんの詳細")
h=mbti_df.iloc[c-1,0:4]
st.text(h)
st.text(f"{d}さんの詳細")
i=mbti_df.iloc[d-1,0:4]
st.text(i)
st.text(j)
st.text(f"{e}さんの詳細")
j=mbti_df.iloc[d-1,0:4]
st.text(j)
